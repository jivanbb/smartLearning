...
class MY_Controller extends MX_Controller
{
    ...
    public function __construct()
    {
        ...
        if (installation_complete()) {
            date_default_timezone_set(get_option('timezone', 'asia/kathmandu'));
            load_extensions();
...
class Admin extends MY_Controller
{
    ...
    public function render($view = '', $data = array(), $bool = FALSE)
    {
        // $this->load->model('attendance_record/model_oms_attendance_record');
        $this->template->enable_parser(false);
        $this->template->set_partial('content', $view, $data);
        $this->template->build('backend/standart/main_layout', $data);
    }
    ...
    public function upload_file($data = [])
    {
        ...
        $dir = $default['upload_path'] . $default['uuid'];
        if (!is_dir($dir)) {
            mkdir($dir, 0777, true);
        }
    ...
    public function delete_file($data = [])
    {
        $default = [
            ...
            'delete_by' => '',
            'field_name' => 'image',
            'upload_path_tmp' => FCPATH . 'uploads/tmp/',
            'table_name' => 'test',
            'primary_key' => 'id',
        ...
        if (!empty($default['uuid'])) {
            ...
            if ($default['delete_by'] == 'id') {
                ...
                if ($row) {
                    $path = $default['upload_path'] . $row->{$default['field_name']};
                }
                ...
                $path =  $default['upload_path_tmp'] . $default['uuid'] . '/';
                $delete_file = delete_files($path, true);
            }
