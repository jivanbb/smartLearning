...
return $id;
    ...
    public function check_child_data_exist($parent_table, $id, $child_tables,$field){
        foreach ($child_tables as $table => $variable) {
            $this->db->select('' . $variable . '');
            $this->db->from('' . $parent_table . '');
            $this->db->join('' . $table . '', '' . $parent_table . '.'.$field.' = ' . $table . '.' . $variable . '','left');
            $this->db->where(''.$parent_table.'.id',$id);
            return $this->db->get()->row('' . $variable . '');
        }
    }
