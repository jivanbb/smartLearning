...
if (!function_exists('display_custom_menu_module')) {
	function display_custom_menu_module($parent, $level, $menu_type, $ignore_active = false)
	{
		$ci = &get_instance();
		$ci->load->database();
		$ci->load->model('menu/model_menu');
		$menu_type_id = $ci->model_menu->get_id_menu_type_by_flag($menu_type);
		$result = $ci->db->query("SELECT a.id, a.name, a.type, a.active, a.link, Deriv1.Count FROM `sp_module` a  LEFT OUTER JOIN (SELECT parent, COUNT(*) AS Count FROM `sp_module` GROUP BY parent) Deriv1 ON a.id = Deriv1.parent WHERE a.menu_type_id = " . $menu_type_id . " AND a.parent=" . $parent . " " . ($ignore_active ? '' : 'and active = 1') . " order by `sort` ASC")->result();
		...
		$display_menu_dropdown = function ($row = null) {
			return '<div class="dropdown  pull-right">
					    ...
					    <span class="caret"></span></a>
					    <ul class="dropdown-menu">
					      <li><a class="remove-data" href="javascript:void()" data-href="' . admin_site_url('sp_module/delete/' . $row->id) . '"><i class="fa fa-trash btn-action"></i> <span class="text-danger">Delete</span></a></li>
					      <li><a href="' . admin_site_url('sp_module/edit/' . $row->id) . '"><i class="fa fa-pencil btn-action"></i> Edit</a></li> 
					      ' . ($row->type != 'label' ? '
					      <li><a href="' . admin_site_url('sp_module/add/' . app()->uri->segment(4) . '?parent=' . $row->id) . '"><i class="fa fa-level-down btn-action"></i> Add child</a></li>
					      ' : '') . '
					    </ul>
					  </div>';
		};
		if ($result) {
			...
			foreach ($result as $row) {
				if ($row->Count > 0) {
					...
					if ($row->type != 'label') {
						$ret .= '<div class="dd-handle dd3-handle dd-handles"></div>';
						$ret .= '<div class="dd3-content">' . _ent($row->name);
					} else {
						$ret .= '<div class="dd-handle dd3-handle dd-handles dd-handle-label"></div>';
						$ret .= '<div class="dd3-content "><b>' . _ent($row->name) . '</b>';
					}
					...
					$ret .= '</div>';
					$ret .= display_custom_menu_module($row->id, $level + 1, $menu_type, $ignore_active);
					$ret .= "</li>";
				} elseif ($row->Count == 0) {
					$ret .= '<li class="dd-item dd3-item ' . ($row->active ? '' : 'menu-toggle-activate_inactive') . ' menu-toggle-activate" data-id="' . $row->id . '" data-status="' . $row->active . '">';
...
if (!function_exists('display_customized_menu_admin')) {
	function display_customized_menu_admin($parent, $level)
	{
		...
		if($teacher_role){
			$result = $ci->db->query("SELECT a.id, a.name,a.icon_color, a.type, a.link,a.icon,a.sub_menu, Deriv1.Count FROM `sp_module` a  LEFT OUTER JOIN (SELECT parent, COUNT(*) AS Count FROM `sp_module` GROUP BY parent) Deriv1 ON a.id = Deriv1.parent WHERE a.menu_type_id = 1 AND id!=6 AND a.parent=" . $parent . " and active = 1  order by `sort` ASC")->result();
		}else{
			$result = $ci->db->query("SELECT a.id, a.name,a.icon_color, a.type, a.link,a.icon,a.sub_menu, Deriv1.Count FROM `sp_module` a  LEFT OUTER JOIN (SELECT parent, COUNT(*) AS Count FROM `sp_module` GROUP BY parent) Deriv1 ON a.id = Deriv1.parent WHERE a.menu_type_id = 1 AND a.parent=" . $parent . " and active = 1  order by `sort` ASC")->result();
		}
			...
			foreach ($result as $row) {
					...
					if ($row->type == 'label') {
						if (check_role_exist_or_not($row->id, array("view", "list", "add", "edit"))) {
							// if ($ci->aauth->is_allowed($perms)) {
							$ret .= '<li class="header treeview">' . _ent(cclang($row->name)) . '</li>';
							// }
						}
					} else {
						if ($row->Count > 0) {
							...
							if (check_role_exist_or_not($row->id, array("view", "list", "add", "edit"))) {
								...
								if ($parent) {
									$ret .= '<i class="fa ' . _ent($row->icon) . ' ' . _ent($row->icon_color) . '"></i> <span>' . _ent(cclang($row->name)) . '</span>
                                <span class="pull-right-container">
                                <i class="fa fa-angle-left pull-right"></i>
                                </span>
                                </a>';
							...
							if ($row->sub_menu == 1) {
								...
								if (check_role_exist_or_not($row->id, array("add"))) {
									if ($language == "nepali") {
										$create = cclang($row->name) . ' ' . cclang('create');
									} else {
										$create = cclang('create') . ' ' . cclang($row->name);
									}
									$ret .= '<li class="' . $active . ' "> 
                                <a href="' . $link . "/add" . '">';

									if ($parent) {
										$ret .= '<i class="fa ' . _ent($row->icon) . ' ' . _ent($row->icon_color) . '"></i> <span>' . _ent($create) . '</span>
                                    <span class="pull-right-container"></i>
                                    </span>
                                    </a>';
									}
                                ...
                                <a href="' . $link . '">';
									if ($parent) {
										$ret .= '<i class="fa ' . _ent($row->icon) . ' ' . _ent($row->icon_color) . '"></i> <span>' . _ent(cclang($row->name) . ' ' . cclang('list')) . '</span>
                                    <span class="pull-right-container"></i>
                                    </span>
                                    </a>';
									}
...
if (!function_exists('get_custom_menu')) {
	function get_custom_menu($menu_type = null)
	{
		$ci = &get_instance();
		$ci->load->database();
		$ci->load->model('menu/model_menu');
...
?>
			<option <?= $child->id == $parent ? 'selected="selected"' : ''; ?> value="<?= $child->id; ?>"><?= str_repeat('----', $tree) ?> <?= ucwords($child->name); ?></option>
