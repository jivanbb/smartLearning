          ...
          <div class="col-md-12" id="box-container">
            <div class="box box-warning">
              <div class="box-body ">
                      ...
                      <div class="input-field-part">
                        <select class="form-control chosen-select-deselect" name="group_id" id="group_name" data-placeholder="<?= cclang('role') ?>">
                          ...
                          <?php foreach (db_get_all_data('aauth_groups') as $row): ?>
                            <option <?= $row->id ==  $department_id ? 'selected' : ''; ?> value="<?= $row->id ?>"><?= $row->name; ?></option>
                          <?php endforeach; ?>
                        </select>
                ...
                <div class="table-responsive">
                  <table id="role_management" class="table table-bordered table-striped dataTable">
                    ...
                    <tbody id="tbody_oms_documents">
                      ...
                      ?>
                          <tr>
                            <td><input type="checkbox" class="check_row" data-level="1"></td>
                            <td><?php echo cclang($module->name); ?></td>
                            <td> <?php if ($module->add == 1) { ?>
                                <input type="checkbox" name="permission[<?php echo $module->id; ?>][add]"
                                  <?php if (@$roles[$module->id]['add'] == 1) {
                                      echo "checked ";
                                    };
                                    echo check_enable_or_not($add_array, $module->name); ?> value="1" class="check_one"> <?php } ?>
                            </td>
                            <td> <?php if ($module->list == 1) { ?><input type="checkbox" name="permission[<?php echo $module->id; ?>][list]" <?php if (@$roles[$module->id]['list'] == 1) {
                                                                                                                                                echo "checked ";
                                                                                                                                              };
                                                                                                                                              echo check_enable_or_not($list_array, $module->name); ?> value='1' class="check_one"></td><?php } ?>
                          <td> <?php if ($module->view == 1) { ?> <input type="checkbox" name="permission[<?php echo $module->id; ?>][view]" <?php if (@$roles[$module->id]['view'] == 1) {
                                                                                                                                                echo "checked ";
                                                                                                                                              };
                                                                                                                                              echo check_enable_or_not($view_array, $module->name); ?> value='1' class="check_one"> </td><?php } ?>


                        <td> <?php if ($module->edit == 1) { ?> <input type="checkbox" name="permission[<?php echo $module->id; ?>][edit]" <?php if (@$roles[$module->id]['edit'] == 1) {
                                                                                                                                              echo "checked ";
                                                                                                                                            };
                                                                                                                                            echo check_enable_or_not($edit_array, $module->name); ?> value='1' class="check_one"> <?php } ?> </td>

                        <td><?php if ($module->delete == 1) { ?> <input type="checkbox" name="permission[<?php echo $module->id; ?>][delete]" <?php if (@$roles[$module->id]['delete'] == 1) {
                                                                                                                                                echo "checked ";
                                                                                                                                              };
                                                                                                                                              echo check_enable_or_not($delete_array, $module->name); ?> value='1' class="check_one"><?php } ?> </td>

                        <td><?php if ($module->mobile == 1) { ?>
                            <input type="checkbox" name="permission[<?php echo $module->id; ?>][mobile]" <?php if (@$roles[$module->id]['mobile'] == 1) {
                                                                                                            echo "checked ";
                                                                                                          };
                        ...
                        <td><?php if ($module->self_only == 1) { ?>
                            <input type="checkbox" name="permission[<?php echo $module->id; ?>][self_only]" <?php if (@$roles[$module->id]['self_only'] == 1) {
                                                                                                              echo "checked ";
                                                                                                            };
                                                                                                            echo check_enable_or_not($list_array, $module->name); ?> value='1' class="check_one">
                          <?php } ?>
                ...
                <button id="btn_edit" class="btn btn-flat btn-primary btn_save btn_action btn-success" id="btn_save" data-stype='stay' title="edit (Ctrl+s)"> <?= cclang('edit'); ?></button>
                <?= form_close(); ?>
              </div>
              <!-- /.col -->
            </div>
...
<script>
  $(document).ready(function() {
    ...
    $('.btn_save').click(function() {
      ...
      $.ajax({
          url: BASE_URL + 'administrator/role/update',
          type: 'POST',
          dataType: 'json',
          data: data_post,
        })
...
<?php
function getSubmodule($sub_group, $modules, $roles, $sn = 2, $sub_mark = '-')
{
  $arr = array();
  foreach ($modules as $key => $value) {
    $arr[] = $value->name;
  ...
  foreach ($sub_group as $key => $data) {
    $use = '<tr>';
    $use .= '<td><input type="checkbox" class="check_row" data-level="' . $sn . '"></td>';
    ...
    if ($data->view == 1) {
      if (@$roles[$data->id]["view"] == 1) {
        $use .= '<td><input type="checkbox" name="permission[' . $data->id . '][view]" 
      checked  value="1" class="check_one"></td>';
      } else {
        $use .= '<td><input type="checkbox" name="permission[' . $data->id . '][view]" 
      value="1" class="check_one"></td>';
      }
    ...
    echo $use;
    $level = $sn + 1;
    getSubmodule($data->sub, $modules, $roles, $level, $sub_mark . ' - ');
  }
} ?>