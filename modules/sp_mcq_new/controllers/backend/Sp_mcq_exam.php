...
class Sp_mcq_exam extends Admin
{
	...
	{
		if (!check_role_exist_or_not(15, array("view","list"))) {
		...
		$this->data['results'] = $this->model_sp_mcq_exam->get_mcq_question($filter);
	...
	{
		$question_arr = [];
		$questions = $this->model_sp_mcq_exam->get_questions($id);
		foreach ($questions as $res) {
			if ($res->correct_option == 1) {
				$correct_ans = $res->option_1;
			}elseif($res->correct_option == 2){
				$correct_ans = $res->option_2;
			}elseif($res->correct_option == 3){
				$correct_ans = $res->option_3;
			}else{
				$correct_ans = $res->option_4;
			}
			// $question_arr[] = [
			...
			$question_arr[] = [
				'question' => $res->question,
				'answer'=>$correct_ans,
				'options'=>[$res->option_1,$res->option_2,$res->option_3, $res->option_4],
				'explanation'=>$res->explanation
			];
		}
		$this->data['questions'] = json_encode($question_arr);
		$this->render('backend/standart/administrator/sp_mcq_exam/sp_question_practice', $this->data);
	}

	public function getCourse($teacher)
	{
		$data = $this->model_sp_mcq_exam->getCourseByTeacher($teacher);
