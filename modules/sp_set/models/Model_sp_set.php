...
class Model_sp_set extends MY_Model
{

    private $primary_key    = 'id';
    private $table_name     = 'sp_set';
    ...
    public function count_all($id, $role, $q = null, $field = null)
    {
        ...
        if (empty($field)) {
            ...
            $where .= "(" . "sp_set." . $field . " LIKE '%" . $q . "%' )";
        }

        $this->join_avaiable()->filter_avaiable();
        $this->db->where($where);
        if ($role == "self_only") {
            $this->db->where('sp_set.created_by', $id);
        }
    ...
    public function check_already_name($name, $id)
    {
        ...
        $this->db->where('is_deleted',0);
        return $this->db->get('sp_set')->row();
    }
}

/* End of file Model_sp_set.php */
/* Location: ./application/models/Model_sp_set.php */