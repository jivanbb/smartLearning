...
class User extends Admin
{
	...
	{
		if (!check_role_exist_or_not(2, array("add","edit","view","list"))) {
	...
	{
		if (!check_role_exist_or_not(2, array("delete"))) {
			$this->session->set_flashdata('f_message', 'Sorry you do not have permission to access ');
			$this->session->set_flashdata('f_type', 'warning');
			redirect('administrator/dashboard','refresh');
		}
	...
	public function edit_profile_save($id)
	{
		...
		if ($this->form_validation->run()) {
			...
			if ($save_user) {
				...
				$this->data['id'] 	   = $id;
				$this->data['message'] = 'Your data has been successfully updated into the database.';
			} else {
